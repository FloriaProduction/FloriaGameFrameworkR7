...
'''Вызывается после создания батчей'''
on_setup_batch = AsyncEvent[Abc.Batch]()
'''Вызывается при создании для каждого батча'''

on_clear = AsyncEvent[Managers.BatchObjectManager]()
'''Вызывается перед очисткой всех батчей'''
on_clear_batch = AsyncEvent[Abc.Batch]()
'''Вызывается при очистки для каждого батча'''
...
def _(window: Abc.Window):
    ...
    @window.on_camera_change.Register
